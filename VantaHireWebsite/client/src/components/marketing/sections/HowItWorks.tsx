...
export default function HowItWorks() {
  ...
  const lockedRef = useRef(false);    // is page scroll currently hijacked
  const canEngageRef = useRef(true);  // hysteresis guard against instant re-lock
  const lockYRef = useRef(0);         // scrollY the page is frozen at while locked
  ...
  useEffect(() => {
    ...
    const ENTER_TH = 0.1;       // engage when section center within 10% vh of center
    const EXIT_TH = 0.45;       // re-arm only after section leaves center by 45% vh
    ...
    const release = (dir: 1 | -1) => {
      ...
      canEngageRef.current = false; // don't immediately re-lock
      // nudge just past the freeze point so the next gesture scrolls the page
      window.scrollTo(0, lockYRef.current + dir * 8);
    };
    ...
    const onScroll = () => {
      ...
      if (lockedRef.current) {
        window.scrollTo(0, lockYRef.current); // keep frozen
  ...
  // Clicking a tab centers the section, locks, and jumps straight to that layer.
