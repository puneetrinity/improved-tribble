...
if (process.env.FLOW_ORGANIZATION_CANDIDATE_TEST === "1") {
  await import("../../organization-candidates/__tests__/lifecycle.pg.test");
}
...
const PRE_4B_APPLIED_RESULT = {
  applied: ["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009"],
};

function pre4BForwardMigrationName(name: string): string {
  const file = `0010_${name}.sql`;
  return file;
}
...
function withForwardMigration(name: string, sql: string): string {
  ...
  for (const file of [
    ...
    "0005_privilege_authorization_version.sql",
    "0006_versioned_invitation_grants.sql",
    "0007_decision_event_spine.sql",
    "0008_decision_projection_outbox.sql",
    "0009_decision_projection_delivery_state.sql",
    "0010_organization_private_candidate_reference.sql",
    "0011_candidate_consent.sql",
    "catalog.lock.json",
  ]) {
    copyFileSync(join(migrationsDir, file), join(dir, file));
  }
  const file = `0012_${name}.sql`;
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      ...
      migrations: {
        ...
        "0005": currentLock.migrations["0005"],
        "0006": currentLock.migrations["0006"],
        "0007": currentLock.migrations["0007"],
        "0008": currentLock.migrations["0008"],
        "0009": currentLock.migrations["0009"],
        "0010": currentLock.migrations["0010"],
        "0011": currentLock.migrations["0011"],
        "0012": sha256(sql),
      },
    }, null, 2)}\n`,
  );
...
describe.skipIf(!enabled || !databaseUrl)("schema-control disposable PostgreSQL", () => {
  ...
  it("installs the exact baseline once and repeats as a no-op", async () => {
    ...
    expect({ applied: first.applied.filter(version => Number(version) < 10) }).toEqual(PRE_4B_APPLIED_RESULT);
    expect(pre4BForwardMigrationName("historical_probe")).toBe("0010_historical_probe.sql");
    expect(first).toEqual({ identityMode: "fresh", applied: ["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010", "0011"] });
    ...
    try {
      ...
      expect(ledger.rows).toEqual([
        ...
        { version: "0005", apply_mode: "adopted" },
        { version: "0006", apply_mode: "adopted" },
        { version: "0007", apply_mode: "adopted" },
        { version: "0008", apply_mode: "adopted" },
        { version: "0009", apply_mode: "adopted" },
      ...
      const invitationGrants = await client.query(`
        SELECT
          (SELECT COUNT(*)::integer FROM information_schema.columns
            WHERE table_schema='public' AND table_name='organization_invites'
              AND column_name IN ('state','version','cancelled_at','cancelled_by','superseded_at')) AS organization_columns,
          (SELECT COUNT(*)::integer FROM information_schema.columns
            WHERE table_schema='public' AND table_name='hiring_manager_invitations'
              AND column_name IN ('accepted_by_user_id','grant_version','revoked_at','revoked_by')) AS hm_columns,
          (SELECT COUNT(*)::integer FROM pg_constraint WHERE conname IN (
            ...
            'hiring_manager_invitations_revocation_shape_check','hiring_manager_invitations_accepted_user_shape_check'
          )) AS checks,
          (SELECT COUNT(*)::integer FROM pg_indexes WHERE indexname IN (
            'org_invites_pending_email_idx','org_invites_org_state_created_idx',
            'org_invites_token_state_idx','hm_invitations_eligibility_idx'
          )) AS indexes
      `);
      expect(invitationGrants.rows[0]).toEqual({
        organization_columns: 5,
        hm_columns: 4,
        checks: 6,
        indexes: 4,
      });
      const decisionSpine = await client.query(`
        SELECT
          to_regclass('public.decision_events')::text AS relation,
          to_regclass('public.decision_event_sequence')::text AS sequence,
          (SELECT COUNT(*)::integer FROM pg_constraint
            WHERE conrelid='public.decision_events'::regclass) AS constraints,
          (SELECT COUNT(*)::integer FROM pg_indexes
            WHERE schemaname='public' AND tablename='decision_events') AS indexes,
          (SELECT COUNT(*)::integer FROM pg_trigger
            WHERE tgrelid='public.decision_events'::regclass AND NOT tgisinternal
              AND tgenabled<>'D') AS triggers,
          (SELECT COUNT(*)::integer FROM decision_events) AS inferred_rows
      `);
      expect(decisionSpine.rows[0]).toEqual({
        relation: "decision_events",
      ...
      const decisionOutbox = await client.query(`
        SELECT
          to_regclass('public.decision_projection_outbox')::text AS relation,
          to_regclass('public.decision_projection_outbox_sequence')::text AS sequence,
          (SELECT COUNT(*)::integer FROM pg_constraint
            WHERE conrelid='public.decision_projection_outbox'::regclass) AS constraints,
          (SELECT COUNT(*)::integer FROM pg_indexes
            WHERE schemaname='public' AND tablename='decision_projection_outbox') AS indexes,
          (SELECT COUNT(*)::integer FROM pg_trigger
            WHERE tgrelid='public.decision_projection_outbox'::regclass AND NOT tgisinternal
              AND tgenabled<>'D') AS triggers,
          (SELECT COUNT(*)::integer FROM decision_projection_outbox) AS inferred_rows
      `);
      expect(decisionOutbox.rows[0]).toEqual({
  ...
  it("applies a representative forward migration exactly once", async () => {
    ...
    expect(upgraded.applied).toEqual(["0012"]);
  ...
  it("serializes two concurrent release runners", async () => {
    ...
    expect([...a.applied, ...b.applied]).toEqual(["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010", "0011"]);
    const client = await directClient();
    try {
      const applied = await client.query("SELECT COUNT(*)::integer AS n FROM schema_control.applied");
      expect(applied.rows[0]?.n).toBe(loadManifest(migrationsDir).length);
  ...
  it("rolls back a failed forward migration and blocks readiness", async () => {
    ...
    try {
      ...
      expect(state.rows[0]).toEqual({ leaked: null, applied: 12 });
      await expect(
        assertSchemaReady({
  ...
  it("provisions the restricted role and adopts legacy baseline with control-only mutation", async () => {
    ...
    try {
      ...
      expect(privacyUpgrade).toEqual({ identityMode: "adopted", applied: ["0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010", "0011"] });
      const preReconciliationRuntime = await runtimeClient();
      try {
        await preReconciliationRuntime.query("BEGIN READ ONLY");
        await expect(assertSchemaReady({
          pg: { query: (text, params) => preReconciliationRuntime.query(text, params as any) },
          migrationsDir,
          environment: "production",
          expectedTargetId: adoptedTarget,
          criticalPostconditions: FLOW_CRITICAL_POSTCONDITIONS,
        })).rejects.toThrow(/Candidate consent tables, column privileges and four routines are exact/);
        await preReconciliationRuntime.query("ROLLBACK");
      } finally {
        await preReconciliationRuntime.end();
      }
      await expect(provision()).resolves.toEqual({ controlPlaneReady: true });
      ...
      expect(forward).toEqual({ identityMode: "adopted", applied: ["0012"] });
      ...
      try {
        ...
        expect(ready).toEqual({ version: "0012", applied: 13 });
