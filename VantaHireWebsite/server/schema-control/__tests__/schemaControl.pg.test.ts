...
describe.skipIf(!enabled || !databaseUrl)("schema-control disposable PostgreSQL", () => {
  ...
  it("installs the exact baseline once and repeats as a no-op", async () => {
    ...
    expect({ applied: first.applied.slice(0, -1) }).toEqual(PRE_4B_APPLIED_RESULT);
    expect(pre4BForwardMigrationName("historical_probe")).toBe("0010_historical_probe.sql");
    expect(first).toEqual({ identityMode: "fresh", applied: ["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010"] });
  ...
  it("applies a representative forward migration exactly once", async () => {
    ...
    expect(upgraded.applied).toEqual(["0011"]);
  ...
  it("serializes two concurrent release runners", async () => {
    ...
    expect([...a.applied, ...b.applied]).toEqual(["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010"]);
    const client = await directClient();
    try {
  ...
  it("rolls back a failed forward migration and blocks readiness", async () => {
    ...
    try {
      ...
      expect(state.rows[0]).toEqual({ leaked: null, applied: 11 });
      await expect(
        assertSchemaReady({
  ...
  it("provisions the restricted role and adopts legacy baseline with control-only mutation", async () => {
    ...
    try {
      ...
      expect(privacyUpgrade).toEqual({ identityMode: "adopted", applied: ["0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010"] });
      ...
      try {
        ...
        })).rejects.toThrow(/Decision-projection delivery functions and runtime boundary/);
      ...
      expect(forward).toEqual({ identityMode: "adopted", applied: ["0011"] });
      ...
      try {
        ...
        expect(ready).toEqual({ version: "0011", applied: 12 });
