...
describe("read-only readiness", () => {
  ...
  it("checks the exact decision-projection outbox relation, sequence, function, and enabled triggers read-only", async () => {
    const check = FLOW_CRITICAL_POSTCONDITIONS.find(
      (postcondition) => postcondition.name === "Decision-projection outbox and append-only guards are exact",
    )!.check;
    let observedSql = "";
    await expect(check({
      async query(text: string) {
    ...
    expect(observedSql).toContain("to_regclass('public.decision_projection_outbox')");
    expect(observedSql).toContain("to_regclass('public.decision_projection_outbox_sequence')");
    expect(observedSql).toContain("flow_reject_decision_projection_outbox_mutation");
    expect(observedSql).toContain("decision_projection_outbox_append_only");
