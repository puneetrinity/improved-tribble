...
describe.skipIf(!enabled)("decision-event spine exact-schema PostgreSQL", () => {
  beforeAll(async () => {
    ...
    const fullUpgrade = await runReleaseMigration({
      migrationsDir,
      creds: { migrateUrl: migrationUrl, expectedTargetId: targetId, environment: "development", allowFreshInitialization: true },
      connect: connectMigration,
    });
    ...
    if (upgrade.applied.join(",") !== "0007,0008,0009"
        || fullUpgrade.applied.length !== currentLedger - 7
        || fullUpgrade.applied[3] !== "0010" || fullUpgrade.applied[4] !== "0011" || fullUpgrade.applied.at(-1) !== currentTail) {
